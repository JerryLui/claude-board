...
**Context:** a cue the daemon plays is not a notification, so no Focus, per-app toggle or notification switch can silence it — and entry 18 mispriced the alternative, since entry 15 already re-signs on any `src/` change, making the marginal cost of shipping sound files zero re-approvals. **Decision:** `install.sh` copies `/System/Library/Sounds/*.aiff` into `Contents/Resources` before `codesign`, the payload digest widens to cover them, and the cue is once again the notification's own sound, named per phase on both paths. **Consequences:** every macOS notification control now reaches the cue, at ~4.7 MB of bundle growth, with enumeration and staging required to read the same directory or the picker offers names the bundle cannot resolve, and the picker's own preview deliberately outside that filtering. **Open:** that `soundNamed:` refuses `/System/Library/Sounds` is taken from Apple's documented search path, not measured — measure before implementing, since the decision collapses if a bare name already resolves.

## 21. The per-stage comment hint is deleted, and the toggle carries discoverability alone — 2026-08-05
**Status:** accepted
**Context:** `stageHint` renders a permanent italic line in every stage's kicker, and a four-option `choose-between-rendered-variants` question renders the same sentence four times in the place vertical space is scarcest; the comment-mode toggle in the header was already made visible chrome for exactly this job. **Decision:** delete `stageHint` outright on both stage kinds, along with its CSS and the `body.readonly` rule that hid it, and let the toggle be the only thing that advertises the element-comment gesture. **Consequences:** a first-time reviewer learns the gesture from the toggle or not at all, and three assertions in `test/check-pure.mjs` invert from "the kicker carries the hint" to "no kicker does"; a hover-only variant was rejected as invisible to touch and keyboard.

## 22. The stage lens may record a pick, so a selection control shares a screen with untrusted content — 2026-08-05
**Status:** accepted
**Context:** a variant option's iframe is inert by entry-less but long-standing decision (`.choice-variant .html-stage { pointer-events: none }`), which is what guarantees only a real reviewer click records a pick; the lens added so a mock can be judged at readable size makes that same stage live, and judging a design full size then hunting back for its card is the workflow the widget exists to remove. **Decision:** the lens carries a control that picks the option it was opened from, closing in the same act, and the card's stage stays inert exactly as before; the control names its option, renders in page chrome visibly outside the framed stage, and is unreachable and unpaintable-over by the iframe. **Consequences:** gives up the property that no selection control ever shares a screen with agent-authored content — the residual risk is a mock drawing convincing fake chrome rather than the stage pressing anything, since the opaque origin, the deleted `select` message and the card handler's `button` exclusion all still hold; a view-only lens was recommended and overruled, and a two-press confirm was rejected as re-adding the friction the lens exists to remove.
