...
const DARK = {
  ...
  // Six-hue syntax palette (ADR.md entry 63) --
  ...
  '--accent-ink': '#0a1020',
  '--good': '#56d68a',
  // ADR.md entry 64: the conventional diff add/remove
...
const LIGHT = {
  ...
  '--good': '#007530',
  // ADR.md entry 64 -- see DARK's own comment for the
...
/* ADR.md entry 46: a page board nobody is listening to is uncommentable, so the
   ...
   listening, on ANY board, not only a page one. Three classes rather than one
   because they are three different facts -- an archive is read-only whatever it
   holds; 'page-uncommentable' is about a page board's round being unawaited in
   particular; 'round-uncommentable' is about the round's own CONTENT, awaited or
   not (src/render.mjs sets both at first paint, src/ui.mjs's
   refreshAwaitDisplay/refreshPager keep them true afterwards -- against the
   clock for the first, against every round flip for the second). */
body.readonly .mode-toggle, body.page-uncommentable .mode-toggle, body.round-uncommentable .mode-toggle { display: none; }
...
/* Unlike .mode-toggle, this control stays live in a read-only archive -- an
   ...
   uncommentable page board, and same again for a round with nothing
   Commentable on it: nothing there is answerable or anchorable, but the theme
   of whatever IS on screen is exactly what a reader still wants to change, so
   all three of .mode-toggle's hiding gates above are carved out here. An id
...
body.readonly button#theme-toggle, body.page-uncommentable button#theme-toggle, body.round-uncommentable button#theme-toggle { display: inline-flex; }
...
/* The panel used to grow upward with NO ceiling --
   ...
   stay reachable at every comment count: unbounded growth pushed the panel's own
   ...
   the panel itself: the Tray (variant A, CONTEXT.md) stops growing at roughly
   40% of the viewport height, so the artifact's upper half stays visible under
   it no matter how many comments are queued. 'min()' keeps the older,
   dock-relative ceiling alongside the flat one rather than replacing it --
   tied to the SAME '--round-pager-dock-h' the 'bottom' value already reads (no
   second number to keep in sync) -- as the floor for a viewport short enough
   that 40vh alone would still let the panel's top edge climb into the
   condensed header pill's own headroom (ADR.md entry 40): "the rest of the
   viewport, minus this panel's own bottom offset, minus 72px of headroom for
   the pill plus its own gap". Whichever number is smaller wins, so the panel's
   top edge can never rise above either limit, at ANY comment count.
   ...
   carrying one of its own. 'overflow: hidden' on the panel itself is a
