...
/* The Tray (variant A, CONTEXT.md) used to grow upward with NO ceiling --
   ...
   the Tray itself: it stops growing at roughly
   ...
   that 40vh alone would still let the Tray's top edge climb into the
   ...
   the pill plus its own gap". Whichever number is smaller wins, so the Tray's
...
/* The scrollable half: only the LIST of already-left comments grows without
   bound and scrolls internally once it outgrows the Tray's own max-height
