...
import {
  roundPageLabel, roundNumberLabel, isPageRound, roundHasCommentable,
...
// What a round that closed WITHOUT being sent says for itself. Its sibling
// PAGE_SEND_EXPIRED_LABEL (src/badge.mjs) covers the other closed-without-sending
// case -- a wait that simply ran out -- and promises the queued comments are
// stored and reach the next agent, which is true there (the round stays `open`,
// so `flushPendingOnExpiry` can still post them). It is FALSE here: a round
// closed by `abandonOpenRounds` leaves the board with nothing `open` on it, every
// submit against it is answered 409, and nothing will ever post to that board
// again -- so a comment still queued in this tab genuinely never left it. Two
// labels rather than one because the difference is exactly what the reviewer
// needs to know, and a frozen control saying the wrong one is worse than a frozen
// control saying nothing.
//
// Declared as real top-level constants, not text inside the `ui` template literal
// below, for the reason MERMAID_TOKEN_MAP's own comment gives: spliced in via
// JSON.stringify so the page and the check can never drift, and directly
// importable by the check that asserts them. They live here rather than beside
// PAGE_SEND_EXPIRED_LABEL in src/badge.mjs because nothing renders them
// server-side -- only this client, and only once the daemon has told it the round
// closed.
export const ROUND_ABANDONED_LABEL = 'Round closed, not sent';
export const ROUND_ABANDONED_TITLE =
  'The conversation that opened this board ended, so this round closed. Comments queued here were never sent.';
...
(function () {
  ...
  var roundHasCommentable = ${roundHasCommentable.toString()};
  ...
  // share one label.
  var ROUND_ABANDONED_LABEL = ${JSON.stringify(ROUND_ABANDONED_LABEL)};
  var ROUND_ABANDONED_TITLE = ${JSON.stringify(ROUND_ABANDONED_TITLE)};
