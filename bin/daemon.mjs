...
//
// Shutdown has to be prompt, because it is not rare: WatchPaths reloads the daemon
// whenever config sync pulls new code, KeepAlive restarts it on crash, and the
// revive command in every unreachable-daemon message is a kickstart. `server.close()`
// alone never gets there — it stops accepting new connections and then waits for the
// open ones to end, and an SSE stream (/api/board/:id/events) never ends by design.
// launchd would SIGTERM, watch us ignore it, and SIGKILL after ExitTimeOut, so every
// reload cost ~20s of total outage — during which every `ask` fails — and ended in an
// unclean kill that can land mid-write. So: stop accepting, then destroy the open
// connections outright (an SSE client reconnects natively; a shim waiter reattaches by
// board id), with a short forced-exit timer as the backstop.
//
