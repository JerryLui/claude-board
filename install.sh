...
# canvas, centered on transparency, because macOS applies the corner shape and shadow
# itself and a body baked in at full bleed is what used to shrink this icon onto a grey
# plate everywhere macOS shows it. The body is ALSO rounded to roughly the standard
# macOS corner radius (184.32px, 0.18 of the 1024 canvas) — belt-and-braces, not a
# corner shape the artwork asserts over the system's own: on a surface that masks a
# legacy icns to its own squircle this radius is invisible underneath it, and on one
# that only pads and shadows without masking it keeps the tile from rendering as a
# sharp square. Regenerated by:
